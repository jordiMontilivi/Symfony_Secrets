...
## 5. Netejar els fitxers `.env` (Molt Important!)
 ...
 Cal netejar els valors de les variables del `.env`, `.env.local` i `.env.prod` i **comentar** els valors d'aquelles variables que acabes de ficar al secrets (vault):
...
Al pujar el codi a GitHub hem de limitar les dades que hem de compartir amb el repositori a través dels `.gitignore`. Evidentment, els mètodes segurs de contrasenyes s'han d'ignorar durant els commits:
...
- Les variables d'entorn sensibles `.env.local`

**Com s'ho farà la màquina virtual EC2 de Producció Sense Aquests Arxius?**

1. Quan executis els scripts a través de GHActions (`full-setup-pro.sh`), es descarregarà, instal·larà i configurarà tot el necessari (si no ho estava ja) per a tenir el teu servidor en un entorn de prod o dev segons on es faci el desplegament.

2. Com a pas manual (o part d'un escriptura de CI/CD via Secret Variables al GH), l'administrador del sistema només s'ha de connectar 1 vegada per scp/SSH a la instància d'EC2 i copiar 3 arxius de les claus privades manuals al disc dur de producció:  
    - `[projecte]/config/secrets/prod/prod.decrypt.private.php`
    - `[projecte]/config/jwt/private.pem`
    - `[projecte]/.env.local`
  
De manera transparent i instantània, en arrancar localment a AWS el servidor Symfony trobarà el fitxer privat `prod.decrypt.private.php`, desencriptarà els valors dels secrets al vol només a memòria, recuperant el de la BBDD de l'RDS i la contrasenya JWT.  
Amb això l'aplicació Producció ja és Plenament Segura! 

<!-- Si voleu un punt més de Sincronisme amb la clau publica de jwt podeu no moure la clau pública i generar-la a partir de la privada que ja està al servidor pujada manualment al primer desplegament... Fent una última connexió al servidor Apache (EC2) des del runner de github podem dir que genere les dues claus privades a partir de les publiques

```yaml
- name: Generate public key on server
  run: |
    ssh ec2-user@your-server '
      openssl rsa -in /var/www/project/config/jwt/private.pem \
      -pubout -out /var/www/project/config/jwt/public.pem
    '
``` -->
